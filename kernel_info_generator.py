...
import os
import re
from loop_statistics import LoopStat
...
LLVM_ISA = "llvm_reference_files/llvm_instruction_set.txt"
...
p = subprocess.Popen(['make','-f',"makefiles/makefile_simulate"])
p.wait()
...
with open(LLVM_TEMP_FILE,"w") as llvm_tf:
    p = subprocess.Popen(["make","-f","makefiles/makefile_opt",f"{DIRNAME}/{STAT_FILE}"],stdout=llvm_tf)
    p.wait()
